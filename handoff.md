...
**改动文件**：`src/components/OptionsPositions.jsx`

**需求**：到期前做了 rolling 的合约，原来要手动「平旧 + 开新」两步，现改为一次操作完成。

**实现**：
- 新增 `RollModal` 组件（在 `CloseModal`/`ExerciseModal` 之后）：弹窗分「旧合约信息（只读）」「平掉旧合约（买回价/日期/手续费）」「开新合约（类型/方向/行权价/到期日/合约数/权利金/手续费）」三块 + 预览
- 操作列（策略组合 & 独立单腿两处）在「平仓」旁新增蓝色「滚动」按钮，仅 `!isClosed` 显示
- `handleRoll`：先 `CLOSE_OPTIONS_POSITION` 平旧合约（自动算已实现盈亏，含开仓+平仓手续费）→ 再 `ADD_OPTION` 开新合约
- 新合约自动关联：`rolledFrom` 字段 + `note` 自动填「Rolled from {symbol} {type} ${strike} {expiration}」，方向/类型/合约数默认继承旧合约，可改
- 预览实时显示「旧合约已实现盈亏」「新合约权利金收入」；必填项（买回价/行权价/到期日/权利金/合约数）未填时「确认滚动」禁用
- 导入 `X` 图标

**验证**：`npx vite build` 通过（744ms）；Playwright 实测 IREN Put $45 滚动：买回 $3.00（权利金 $2.55、开仓手续费 $0.67）→ 旧合约盈亏 -$45.67 正确，新合约 Put $40 权利金 $2.80 → 收入 +$280.00 正确，到期日未填时按钮禁用、填后启用。未污染真实数据。

---

## 历史状态（2026-08-14，第四十九次）✅
